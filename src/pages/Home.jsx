...
export default function Home() {
  ...
  const fetchSuggestions = (input) => {
    ...
    fetchPlaceSuggestions(autocompleteServiceRef.current, input, 'ke').then((predictions) => {
  ...
  const handleSelectSuggestion = (suggestion) => {
    ...
    setWebLoading(true);
    resolveDestinationAndNearbyParking(apiKey, suggestion)
      .then(({ destination, webParking: web }) => {
        if (destination) {
  ...
  const handleBook = (spot) => {
    if (spot.isExternal && spot.location) {
      const url = buildExternalBookingUrl(spot);
      window.open(url, '_blank');
      return;
    }

    // TODO: Integrate with internal booking flow for provider-owned lots
  };
      ...
      <div className="px-5 mt-6 max-w-lg">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-bold text-gray-900">
            {selectedDestination && webParking.length > 0
              ? `Available Parking near ${selectedDestination.name}`
              : 'Available Parking'}
          </h2>
        </div>
        ...
        {effectiveLoading ? (
          ...
          <div className="bg-white rounded-2xl p-8 text-center border border-gray-100">
            ...
            <p className="text-gray-400 text-xs mt-1">
              {selectedDestination
                ? 'No parking found from Google around this area yet.'
                : 'New providers will appear here once approved.'}
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            {effectiveLots.map((spot) => {
