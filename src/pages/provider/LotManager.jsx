...
import { MapPin, DollarSign, Car, Save, CheckCircle2, ParkingCircle, Edit3, Plus, X, Link, Upload } from 'lucide-react';

const MAX_IMAGES = 4;

/**
 * Resize and compress an image file to base64.
 * Keeps max dimension 800px, JPEG 70% quality → ~60–100KB per image.
 */
...
export default function LotManager() {
    ...
    const existingImages = userProfile?.lotImages || (userProfile?.businessImage ? [userProfile.businessImage] : []);
    ...
    const [lotImages, setLotImages] = useState(existingImages);
    ...
    async function handleSave(e) {
        ...
        try {
            ...
            await updateUserProfile(currentUser.uid, {
                ...
                lotImages,
                businessImage: lotImages[0] || userProfile?.businessImage || '',
            });
            setSaved(true);
    ...
    const hasSetup = userProfile?.capacity && userProfile?.hourlyRate;

    /** Renders the image grid for the display card */
                ...
                <div className="px-5 pt-12 lg:pt-8 max-w-lg">
                    ...
                    {!hasSetup || editing ? (
                            ...
                            <form onSubmit={handleSave} className="p-5 space-y-3.5">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] text-gray-400 mt-1">
                                        Upload photos or paste URLs. First image becomes the cover. Min 720p recommended.
                                    </p>
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" value={formData.businessLocation}
                                        ...
                                        className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-500 text-sm"
                                        placeholder="e.g. Ngong Road, Nairobi" required />
                                </div>
                        ...
                        <div className="bg-white rounded-2xl overflow-hidden border border-gray-100">
                            {/* Image Grid — all images visible at once */}
                            <ImageGrid images={existingImages} />

                            <div className="p-5">
                                <h2 className="text-lg font-bold text-gray-900 mb-1">{userProfile?.businessName}</h2>
                                <p className="text-gray-400 text-xs flex items-center gap-1 mb-4">
                                    <MapPin className="w-3 h-3" /> {userProfile?.businessLocation}
                                </p>
                                {userProfile?.description && (
                                    <p className="text-gray-500 text-sm mb-4">{userProfile.description}</p>
                                )}
                                <div className="grid grid-cols-3 gap-2">
                                    <div className="bg-gray-50 rounded-xl p-3 text-center">
                                        <p className="text-[10px] text-gray-400 uppercase">Capacity</p>
                                        <p className="text-lg font-bold text-gray-900">{userProfile?.capacity || '—'}</p>
                                    </div>
                                    <div className="bg-gray-50 rounded-xl p-3 text-center">
                                        <p className="text-[10px] text-gray-400 uppercase">Available</p>
                                        <p className="text-lg font-bold text-emerald-600">{userProfile?.availableSpots ?? userProfile?.capacity ?? '—'}</p>
                                    </div>
                                    <div className="bg-gray-50 rounded-xl p-3 text-center">
                                        <p className="text-[10px] text-gray-400 uppercase">Rate/hr</p>
                                        <p className="text-lg font-bold text-teal-600">KSh {userProfile?.hourlyRate || '—'}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
